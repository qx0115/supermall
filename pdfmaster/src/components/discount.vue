...
<div class="discount">
    ...
    <ul >
        ...
        <li class="fun_li"><img src="../assets/css/discount/webcopy.svg" alt=""><span class="fun_title">Powerful editing tools</span><span class="fun_body">Feature-rich software to meet all <br> your PDF needs</span></li>
...
<img src="../assets/css/discount/Waimakariri.png" alt="" class="bgc_i">
 <div class="dis_download_total" @click="downloadfree_ddown()">
            <img src="../assets/css/discount/Group16.png" alt="" class="dis_download_png">
            <span class="download_total_num">{{downloadNum}}

            </span>
            <span class="download_ddownload">
                Downloads
            </span>
        </div>
...
<!-- 促销后 -->
<span class="discount_wenan">   3-Day free trail, automatically renew, cancel any time.</span>
...
if(this.$store.state.seconds!=null){
 this.seconds=this.$store.state.seconds
  ...
  mounted(){
    if(this.$store.state.downloadNum!==null){
               
               this.downloadNum=this.$store.state.downloadNum
            }else{
               this.downloadNum=558603
            }
           setInterval(()=>{    
              this.downloadNum=parseInt(this.downloadNum) + Math.round(Math.random()*4)
              this.$store.commit('set_downloadNum',this.downloadNum)
           },3000)
...
methods:{
    ...
    async buydiscount(){
        fbq('trackCustom', 'payDiscount',{currency: "USD", value: 30.99})
        fbq('track', 'Purchase', {currency: "USD", value: 30.99});
        const params= {
                   hitType: 'event',
